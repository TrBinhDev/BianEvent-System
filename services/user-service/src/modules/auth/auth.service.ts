...
import { hashPassword, comparePassword, hashSHA256, compareSHA256 } from '../../utils/hash'
import { signAccessToken, signRefreshToken, verifyRefreshToken } from '../../utils/token'
import { generateOtp, verifyOtp } from '../../utils/otp'
import { AppError } from '../../middlewares/error-handler'
import { kafkaProducer } from '../../kafka/producer'
import type { RegisterDto, LoginDto, ForgotPasswordDto, ResetPasswordDto } from './auth.dto'
...
export const register = async (dto: RegisterDto) => {
  ...
  return { userId: user.id, message: 'Đăng ký thành công, vui lòng kiểm tra email để xác thực' }
}
...
export const verifyEmail = async (userId: string, otp: string) => {
  ...
  if (user.status === 'ACTIVE') throw new AppError(400, 'Tài khoản đã được xác thực')
...
export const resendOtp = async (userId: string) => {
  ...
  if (isRateLimited) throw new AppError(429, 'Vui lòng chờ 1 phút trước khi gửi lại')
...
export const login = async (dto: LoginDto) => {
  ...
  if (user.status === 'UNVERIFIED') throw new AppError(403, 'Vui lòng xác thực email trước khi đăng nhập')
    ...
    messages: [
      {
        value: JSON.stringify({
          to: user.email,
          type: 'RESET_PASSWORD',
          data: { resetLink: `${process.env.WEB_URL}/reset-password?token=${resetToken}` },
        }),
      },
    ],
  })
